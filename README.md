...
> **Note**: For custom ports or remote servers, set `-e HOST_PORT=<port>` and `-e HOST_NAME=<hostname>` so logs display correct URLs (e.g., `docker run -d --privileged -e HOST_PORT=9999 -e HOST_NAME=myserver.com -p 9999:8080 --name wiki-cluster wiki-cluster`).
...
while true; do
  # 60% chance to create post, 40% to create user
  r=$(( RANDOM % 100 + 1 ))

  if [ $r -le 60 ]; then
    echo "[POST] Probability hit: $r"
    curl -X POST http://localhost:8080/posts \
      -H "Content-Type: application/json" \
      -d "{\"user_id\": 1, \"content\": \"Post at $(date)\"}"
  else
    echo "[USER] Probability hit: $r"
